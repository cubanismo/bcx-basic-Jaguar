'_______________DEATH SWORD_____________
'__________palace software 1988_________
'____a remake of ZX SPECTRUM version____
...
'_______sound librairy by zerosquare____
'_______________________________________
...
dim NoSPRarbreD = 9
dim NoSPRfeusang = 10
dim NoSPRserpentG = 11
dim NoSPRserpentD = 12
dim NoSPRcache1 = 13
dim NoSPRcache2 = 14
dim NoSPRcouleurs = 15
dim NoSPRmenu = 16
dim NoSPRfleche = 17

' chargement des palettes
'loadclut(strptr(decorsZX_clut),0,16) ' palette decors
loadclut(strptr(menuZX_clut),0,256) ' palette menu
loadclut(strptr(paletteA1_clut),6,8) ' spriteA
loadclut(strptr(paletteB1_clut),7,8) ' spriteB
loadclut(strptr(SPRgnome_clut),8,8) ' gnome + petits sprites
...
DO
    ...
    IF GOTOU = 100 THEN
		...
		'cacher les couleurs
		rlist[NoSPRcouleurs].x=-100<<16
		rlist[NoSPRcouleurs].y=300<<16
		
		loadscreen = 0
		temps = temps + 1
		
		rlist[NoSPRmenu].x=19<<16 ' affichage du titre
		rsetobj(NoSPRmenu,R_sprite_gfxbase,(int)strptr(menuZX)+ (67200 * 0) ) ' choix de la frame dans la planche 64000 = 320*(200*8)/8

		' lancer la musique ' ************* attention present 3x dans le code !
		IF musiqueLOOP = 0 THEN SNDZEROPLAY(1, (void *)(sfx_theme), (sfx_theme_end-sfx_theme)+3 and 0xfffffffc, 46168/7694, Zero_Audio_8bit_muLaw|Zero_Audio_Looping)
		' speed of music at 46168/11542 for emulators
		' speed of music at 46168/7694 for jaguar
    ...
    IF GOTOU = 101 THEN
		...
		IF loadscreen = 0 THEN	
			cls
			temps = 0: loadscreen = 1
			
			'cacher les couleurs
			rlist[NoSPRcouleurs].x=-100<<16
			rlist[NoSPRcouleurs].y=300<<16			
			' affichage du titre.	
			'rsetobj(NoSPRmenu,R_sprite_gfxbase,(int)strptr(decorsZX) ) ' choix de la frame dans la planche 32000 = 320*(200*4)/8
			

		END IF

		'RLOCATE 28,8: RPRINTINT temps ' a regler
		IF temps > 3900 THEN ' lancer la demo a la fin de la musique
			loadscreen = 0
			partie = 1:  IA = 4: sorcier = 0: marianna = 0: demo = 1 
            decorS = decorS + 1 ' changer de decor pour la demo
            if decorS > 4 THEN decorS = 1
			GOTOU = 8 
		END IF
		
		if temps > 60 THEN
			IF zero_left_pad band Input_Pad_A THEN loadscreen = 0: temps = 0: GOTOU = 102
			IF zero_left_pad band Input_Pad_B THEN loadscreen = 0: temps = 0: GOTOU = 102
			IF zero_left_pad band Input_Pad_C THEN loadscreen = 0: temps = 0: GOTOU = 102			
		END IF
	...
	IF GOTOU = 102 THEN
		...
		'RLOCATE 28,8: RPRINTINT temps
		temps = temps + 1

		
		' cacher les sprites
		rlist[NoSPRspriteA].x=-80<<16
		rlist[NoSPRspriteB].x=-80<<16	
		rlist[NoSPRspriteAmort].x=-80<<16
		rlist[NoSPRspriteAmort].x=-80<<16
		rlist[NoSPRgnome].x=-32<<16
		rlist[NoSPRombre].x=-32<<16
		rlist[NoSPRtetes].x=-32<<16		
		rlist[NoSPRarbreG].x=-64<<16
		rlist[NoSPRarbreD].x=-64<<16
		rlist[NoSPRfeusang].x=-64<<16
		rlist[NoSPRserpentG].x=-32<<16		
		rlist[NoSPRserpentD].x=-32<<16
		rlist[NoSPRcache1].x=-32<<16
		rlist[NoSPRcache2].x=-32<<16

		'rlist[NoSPRmenu].x=-320<<16
		'rlist[NoSPRfleche].x=-32<<16


		' affichage du menu	et de la musique
		
        IF loadscreen = 0 THEN 
                loadscreen = 1
				
				loadclut(strptr(menuZX_clut),0,256) ' palette decors
				
                hauteur = 148				 
				rsetobj(NoSPRmenu,R_sprite_gfxbase,(int)strptr(menuZX)+ (67200 * 1) ) ' choix de la frame dans la planche 64000 = 320*(200*8)/8		
				rlist[NoSPRmenu].x=19<<16 
				rlist[NoSPRcouleurs].x=80<<16 ' affichage des couleurs
				rlist[NoSPRcouleurs].y=112<<16

		END IF


		IF zero_left_pad band Input_Pad_Up THEN
		   IF temps> 10 THEN
		        temps = 0
		        hauteur = hauteur - 8 
		        IF hauteur < 140 THEN hauteur = 140 
		    END IF
		END IF

		IF zero_left_pad band Input_Pad_Down THEN
		    IF temps > 10 THEN
				temps = 0
				hauteur = hauteur + 8 
				IF hauteur > 180 THEN hauteur = 180
		    	END IF
		END IF

		' affichage de la fleche
		rlist[NoSPRfleche].x=102<<16
		rlist[NoSPRfleche].y=hauteur<<16


		bouton = 0 
		IF zero_left_pad band Input_Pad_A  THEN bouton = 1 
		IF zero_left_pad band Input_Pad_B  THEN bouton = 1 
		IF zero_left_pad band Input_Pad_C  THEN bouton = 1
		
		IF bouton = 1 AND temps >= 30 THEN

            IF hauteur = 140 THEN '  *********demo 
                partie = 1: IA = 4: sorcier = 0: marianna = 0: demo = 1 ' GOTOU = 8 
				decorS = decorS + 1 ' changer de decor pendant la demo
				if decorS > 4 THEN decorS = 1 
                ' couleurs speciales menu spectrum
				temps=0: loadscreen = 0: Ycouleur = 112: GOTOU = 103 				
            END IF
            IF hauteur = 148 THEN '  **********solo 
                partie = 1: decorS = 3: IA = 0: sorcier = 0: marianna = 0: demo = 0 : GOTOU = 8 
                'partie = 1: decorS = 4: IA = 7: sorcier = 0: marianna = 0: demo = 0' GOTOU = 8 '  sorcier
                ' couleurs speciales menu spectrum
				temps=0: loadscreen = 0: Ycouleur = 112: GOTOU = 103			
            END IF
            IF hauteur = 156 THEN '  ***********vs 
                chronometre = 60
                partie = 2: decorS = 2: IA = 0: sorcier = 0: marianna = 0: demo = 0' GOTOU = 8 
                ' couleurs speciales menu spectrum
				temps=0: loadscreen = 0: Ycouleur = 112: GOTOU = 103				
            END IF
            IF hauteur = 164 THEN ' ************controls1 
                temps=0
                loadscreen =0
				Ycouleur = 112
				GOTOU = 103
			END IF
            IF hauteur = 172 THEN ' *************histoire 
                temps=0
                loadscreen =0
				Ycouleur = 112
				GOTOU = 103            
			END IF
            IF hauteur = 180 THEN ' *************credits1 
                temps=0
                loadscreen =0
				Ycouleur = 112
 				GOTOU = 103           
		...
		IF temps > 1500 THEN ' demo si aucun choix
                partie = 1: IA = 4: sorcier = 0: marianna = 0: demo = 1
				decorS = decorS +1 ' changer de decor pendant la demo
				if decorS > 4 THEN decorS = 1
				'GOTOU = 8
                ' couleurs speciales spectrum
				temps=0: loadscreen = 0: Ycouleur = 112: GOTOU = 103				
		END IF
    ...
    '               *********************************************************
    '               ****** effets de couleur sur la police du menu  *********
    '               *********************************************************	
    IF GOTOU = 103 THEN 

        IF loadscreen = 0 THEN             
			loadscreen = 1
			rlist[NoSPRfleche].x=-32<<16 ' cacher la fleche
			rlist[NoSPRcouleurs].x=80<<16 ' affichage des couleurs
			rlist[NoSPRcouleurs].y=112<<16 
		END IF		
		...
		temps = temps + 1
		Ycouleur = Ycouleur - 1
		if Ycouleur <= 42 THEN Ycouleur = 42
		rlist[NoSPRcouleurs].y=Ycouleur<<16 ' faire monter les couleurs

		IF temps > 80 THEN 
                temps=0
                loadscreen =0
				GOTOU = 104
				IF hauteur < 164  THEN GOTOU = 8 ' aller direct au jeu
		END IF
    ...
    '               *********************************************************
    '               ********controls1,controls2, histoire, credits1**********
    '               *********************************************************
    IF GOTOU = 104 THEN 
        ...
        IF loadscreen = 0 THEN
            ...
            IF hauteur = 164 THEN ' controls1 
                 rsetobj(NoSPRmenu,R_sprite_gfxbase,(int)strptr(menuZX)+ (67200 * 2) ) ' choix de la frame dans la planche 64000 = 320*(200*8)/8
            END IF
			
            IF hauteur = 165 THEN ' controls2 
                 rsetobj(NoSPRmenu,R_sprite_gfxbase,(int)strptr(menuZX)+ (67200 * 3) ) ' choix de la frame dans la planche 64000 = 320*(200*8)/8
            END IF
			
            IF hauteur = 172 THEN ' histoire 
                rsetobj(NoSPRmenu,R_sprite_gfxbase,(int)strptr(menuZX)+ (67200 * 4) ) ' choix de la frame dans la planche 64000 = 320*(200*8)/8
            END IF

            IF hauteur = 180 THEN ' affichage des credits1 
                rsetobj(NoSPRmenu,R_sprite_gfxbase,(int)strptr(menuZX)+ (67200 * 5) ) ' choix de la frame dans la planche 64000 = 320*(200*8)/8
            END IF
			
            IF hauteur = 181 THEN ' affichage des credits2 
                rsetobj(NoSPRmenu,R_sprite_gfxbase,(int)strptr(menuZX)+ (67200 * 6) ) ' choix de la frame dans la planche 64000 = 320*(200*8)/8
            END IF
        ...
        IF zero_left_pad band Input_Pad_A  THEN
            IF temps >= 30 THEN temps = 1500 
        END IF
        IF zero_left_pad band Input_Pad_B  THEN
            IF temps >= 30 THEN temps = 1500 
        END IF
        ...
        IF temps >= 1500 THEN
            ...
            IF hauteur = 165 or hauteur = 172 or hauteur = 181 THEN '  retour au menu 
				loadscreen = 0 : temps = 0
				hauteur = 148 : GOTOU = 102 
			END IF
			
			IF hauteur = 164 THEN hauteur = 165 'controls 1>2
			IF hauteur = 180 THEN hauteur = 181 'credits 1>2
			
            IF hauteur = 165 THEN ' aller a controls 2
				loadscreen = 0 : temps = 0
				GOTOU = 104 ' reload 104 
			END IF	
			
            IF hauteur = 181 THEN ' aller a credits 2
				loadscreen = 0 : temps = 0
				GOTOU = 104 ' reload 104 
			END IF	
    ...
    IF GOTOU = 8 THEN
		...
		rlist[NoSPRmenu].x=-320<<16 ' cacher le menu + couleurs + fleche
		rlist[NoSPRcouleurs].x=-100<<16
		rlist[NoSPRcouleurs].y=300<<16		
		rlist[NoSPRfleche].x=-32<<16 
    ...
    IF GOTOU = 32 THEN
		...
		' chargement des palettes de jeu
		loadclut(strptr(decorsZX_clut),0,8) ' palette decors
		loadclut(strptr(paletteA1_clut),6,8) ' spriteA
		loadclut(strptr(paletteB1_clut),7,8) ' spriteB
		loadclut(strptr(SPRgnome_clut),8,8) ' gnome + petits sprites
		...
		' choix du decor
		rsetobj(NoSPRdecor,R_sprite_gfxbase,(int)strptr(decorsZX)+ (32000 * (decorS-1) ) )' choix de la frame dans la planche 32000 = 320*(200*4)/8

			
		'  affichage de l'arbre gauche
		rsetobj(NoSPRarbreG,R_sprite_gfxbase,(int)strptr(arbresGAUCHE)+ ( 2048* (decorS-1) ) )' choix de la frame dans la planche 1536 = 64*(64*4)/8
		rlist[NoSPRarbreG].x=3<<16
			
		'  affichage de l'arbre droit
		rsetobj(NoSPRarbreD,R_sprite_gfxbase,(int)strptr(arbresDROIT)+ ( 2048* (decorS-1) )	)		
		rlist[NoSPRarbreD].x=291<<16		
		...
		if scoreA = 0 THEN RLOCATE 100,60: RPRINT "00000"
		if scoreB = 0 THEN RLOCATE 188,60: RPRINT "00000"
		scoreAactuel = 0' pour re-afficher les scores
		scoreBactuel = 0

		' affichage des points de vie
		RLOCATE 32,20: RPRINT "***"		
		RLOCATE 268,20: RPRINT "***"


		' choix de la palette du GNOME
		if decorS = 1 then loadclut(strptr(paletteA2_clut),8,16)
		if decorS = 2 or decorS = 4 then loadclut(strptr(paletteA1_clut),8,16)
		if decorS = 3 then loadclut(strptr(paletteA3_clut),8,16)
...
sonpoints = 0
		...
		' choix des couleurs des sprites
		if decorS = 1 THEN loadclut(strptr(paletteA2_clut),6,16)
		if decorS = 2 THEN loadclut(strptr(paletteA1_clut),6,16)
		if decorS = 3 THEN loadclut(strptr(paletteA3_clut),6,16)
		if decorS = 4 THEN loadclut(strptr(paletteA1_clut),6,16)

		if decorS = 1 THEN loadclut(strptr(paletteB2_clut),7,16)
		if decorS = 2 THEN loadclut(strptr(paletteB1_clut),7,16)
		if decorS = 3 THEN loadclut(strptr(paletteB3_clut),7,16)
		if decorS = 4 THEN loadclut(strptr(paletteB1_clut),7,16)
	...
	' lancer la musique 
    IF temps > 15 THEN
		'IF pad1 band PAD _C THEN toucheMUSIQUE = 1 ELSE toucheMUSIQUE = 0
		IF zero_left_pad band Input_Pad_C THEN toucheMUSIQUE = 1 ELSE toucheMUSIQUE = 0
	END IF
    ...
    IF GOTOU = 0 THEN 
		...
		temps = temps + 1 ': delaiMUSIQUE = delaiMUSIQUE + 1
    ...
    IF GOTOU = 0 AND jeu = 2 THEN GOTOU = 31
	
    IF GOTOU = 0 AND joueurB = 777 THEN GOTOU = 24
    ...
    IF GOTOU = 0 AND sorcier = 1 THEN
           IF xA >= 26 THEN
			  ...
			  marianna = 1 ' pour cacher marianna			  
			  
			  Aoccupe = 1
              joueurA = 777: spriteA = 0 : xA = 25            
			  
			  
			  Boccupe = 1: reftempsB = temps
              joueurB = 777: xB = 31
    ...
    IF GOTOU = 0 AND bonusA = 1 THEN
        ...
        IF musiqueLOOP = 0 AND sonpoints = 0 THEN
            SNDZEROPLAY(1, strptr(sfx_points), (strptr(sfx_points_end)-strptr(sfx_points)+3) and 0xfffffffc, 46168/9233, Zero_Audio_8bit_muLaw|0)
            sonpoints = 1
        END IF
        ...
        IF joueurB = 95 OR joueurB = 910 THEN
            IF xA >= 39 THEN
                ...
                IF partie = 1 THEN
                    ...
                    ' changement de decor
                    IF GOTOU = 0 THEN
                        IF IA = 0 THEN decorS = 3
                        IF IA = 1 THEN decorS = 4
                        IF IA = 2 THEN decorS = 3
                        IF IA = 3 THEN decorS = 4
                    ...
                    IF GOTOU = 0 AND transitionfinie = 1 THEN GOTOU = 8
                       ...
                       IF GOTOU = 0 THEN
                               IF IA < 7 THEN transition = 1: reftemps = temps: GOTOU = 31
                               IF IA >= 7 THEN transitionfinie = 1: GOTOU = 31 ' /* pastransition pour sorcier */
                       END IF
                ...
                IF GOTOU = 0 AND partie = 2 THEN
                    ...
                    IF GOTOU = 0 AND transitionfinie = 1 THEN
                        ...
                        IF GOTOU = 0 AND decorS = 1 THEN decorS = 2: GOTOU = 32
    ...
    IF GOTOU = 0 AND sorcier = 1 THEN
		...
		IF xA > 6 THEN
            IF xAttB <= xTA AND xAttB > xA THEN
                IF yAttB = yTA THEN
                    gnome = 0
    ...
    IF GOTOU = 0 AND entree = 1 THEN
		'IF lancerintro = 0 THEN ' a la fin sur spectrum
			'lancerintro = 1 
			'IF musiqueLOOP = 0 THEN 
				'SNDZEROPLAY(1, strptr(sfx_theme), (strptr(sfx_theme_end)-strptr(sfx_theme)+3) and 0xfffffffc, 46168/9233, Zero_Audio_8bit_muLaw|0)
			'END IF
		'END IF
    ...
    IF GOTOU = 0 AND entreesorcier = 1 THEN
        IF serpentA = 0 AND xA >= 3 THEN serpentA = 1: refserpentA = temps: serpentB = 1: refserpentB = temps ' faire crier les serpents
		IF GOTOU = 0 AND xA >= 5 THEN xA = 5: entreesorcier = 0: GOTOU = 26
        IF GOTOU = 0 THEN levier1 = 6: GOTOU = 25
    END IF
    ...
    IF GOTOU = 0 AND sens = 6 THEN
        IF levier1 = 6 THEN
            protegeDA = 0
			IF GOTOU = 0 AND attaqueA = 1 AND demo = 0 AND entree = 0 AND entreesorcier = 0 THEN joueurA = 66: Aoccupe = 1: reftemps = temps: GOTOU = 26
    ...
    IF GOTOU = 0 AND sens = 9 THEN
        ...
        IF GOTOU = 0 AND levier1 = 4 THEN
            protegeDA = 0
			IF GOTOU = 0 AND attaqueA = 1 AND demo = 0 THEN joueurA = 96: Aoccupe = 1: reftemps = temps: GOTOU = 26
    ...
    IF GOTOU = 0 AND joueurA = 1 THEN ' attente
        ...
        IF GOTOU = 0 AND temps > reftemps + 20 THEN spriteA = 2: GOTOU = 1
        IF GOTOU = 0 AND temps > reftemps + 11 THEN GOTOU = 1
        IF GOTOU = 0 AND temps > reftemps + 10 THEN  spriteA = 1: GOTOU = 1 '_SNDPLAY attente:
        GOTOU = 1
    END IF
    ...
    IF GOTOU = 0 AND joueurA = 2 THEN 'avance
        ...
        xAttA = xA
        IF GOTOU = 0 AND attaqueA = 1 AND demo = 0 AND entree = 0 AND entreesorcier = 0 THEN joueurA = 66: Aoccupe = 1: reftemps = temps: GOTOU = 26
        spriteA = 4: spriteavanceA = 1: xA = xA + 1
        GOTOU = 1
    END IF
    ...
    IF GOTOU = 0 AND joueurA = 7 THEN 'recule
        ...
        xAttA = xA
        IF GOTOU = 0 AND attaqueA = 1 AND demo = 0 AND sortieA = 0 THEN joueurA = 61400: Aoccupe = 1: reftemps = temps: GOTOU = 26
        spriteA = 6: spritereculeA = 1: xA = xA - 1
        GOTOU = 1
    END IF
    ...
    IF GOTOU = 0 AND joueurA = 615 THEN 'rouladeAV
        ...
        IF GOTOU = 0 AND temps > reftemps + 1 THEN xA = xA + 1: spriteA = 11: GOTOU = 1 '_SNDPLAY roule:
    ...
    IF GOTOU = 0 AND joueurA = 6150 THEN 'rouladeAR
        ...
        IF GOTOU = 0 AND temps > reftemps + 2 THEN GOTOU = 1
        IF GOTOU = 0 AND temps > reftemps + 1 THEN xA = xA - 1: spriteA = 16: GOTOU = 1 '_SNDPLAY roule:
    ...
    IF GOTOU = 0 AND joueurA = 6131 THEN 'protegeH1
        ...
        IF GOTOU = 0 AND temps > reftemps + 2 THEN GOTOU = 1
        IF GOTOU = 0 AND temps > reftemps + 1 THEN xA = xA - 1: spriteA = 4: GOTOU = 1 '_SNDPLAY protege: 
    ...
    IF GOTOU = 0 AND joueurA = 6121 THEN 'protegeD1
        ...
        IF GOTOU = 0 AND temps > reftemps + 2 THEN GOTOU = 1
        IF GOTOU = 0 AND temps > reftemps + 1 THEN GOTOU = 1 '_SNDPLAY protege:
        IF GOTOU = 0 THEN GOTOU = 1
    END IF
    ...
    IF GOTOU = 0 AND joueurA = 63 THEN 'cou
        ...
        IF GOTOU = 0 AND temps > reftemps + 20 THEN
            ...
            IF joueurB = 63 AND spriteB = 20 AND distance < 12 THEN
				...
				IF musiqueLOOP = 0 THEN SNDZEROPLAY(1, strptr(sfx_cling), (strptr(sfx_cling_end)-strptr(sfx_cling)+3) and 0xfffffffc, 46168/9233, Zero_Audio_8bit_muLaw|0)
                'IF choixCLING = 1 THEN _SNDPLAY cling
                'IF choixCLING = 2 THEN _SNDPLAY cling2
                'IF choixCLING = 3 THEN _SNDPLAY cling3
                GOTOU = 1
            END IF
        ...
        IF GOTOU = 0 AND temps > reftemps + 10 THEN
            '_SNDPLAY epee
			''IF musiqueLOOP = 0 THEN SNDZEROPLAY(1, strptr(sfx_epee), (strptr(sfx_epee_end)-strptr(sfx_epee)+3) and 0xfffffffc, 46168/9233, Zero_Audio_8bit_muLaw|0)
    ...
    IF GOTOU = 0 AND joueurA = 68 THEN 'genou
        ...
        yGA = 20
        IF GOTOU = 0 AND temps > reftemps + 30 THEN Aoccupe = 0: joueurA = 61002: assisA = 1: GOTOU = 1
    ...
    IF GOTOU = 0 AND joueurA = 98 THEN 'genouR
        ...
        IF GOTOU = 0 AND temps > reftemps + 30 THEN Aoccupe = 0: joueurA = 9102: assisA = 1: GOTOU = 1
    ...
    IF GOTOU = 0 AND joueurA = 61400 THEN 'decapite
        ...
        IF GOTOU = 0 AND temps > reftemps + 20 THEN
			spriteA = 39: GOTOU = 1'_SNDPLAY decapite:
			'IF musiqueLOOP = 0 THEN SNDZEROPLAY(1, strptr(sfx_epee), (strptr(sfx_epee_end)-strptr(sfx_epee)+3) and 0xfffffffc, 46168/9233, Zero_Audio_8bit_muLaw|0)			
		END IF
        ...
        IF GOTOU = 0 AND temps >= reftemps THEN spriteA = 37: GOTOU = 1
    END IF
...
'coco
    ...
    IF GOTOU = 0 AND joueurA = 620 THEN 'touche
        ...
        IF GOTOU = 0 AND vieA <= 0 THEN
			'IF musiqueLOOP = 0 THEN SNDZEROPLAY(1, strptr(sfx_gnome), (strptr(sfx_gnome_end)-strptr(sfx_gnome)+3) and 0xfffffffc, 46168/9233, Zero_Audio_8bit_muLaw|0)
			serpentA = 0: sangA = 1
    ...
    IF GOTOU = 0 AND joueurA = 6201 THEN 'touche1
        ...
        IF GOTOU = 0 AND temps > reftemps + 2 THEN
			...
			IF musiqueLOOP = 0 THEN SNDZEROPLAY(1, strptr(sfx_touche), (strptr(sfx_touche_end)-strptr(sfx_touche)+3) and 0xfffffffc, 46168/9233, Zero_Audio_8bit_muLaw|0)
            'IF choixTOUCHE = 1 THEN _SNDPLAY touche
            'IF choixTOUCHE = 2 THEN _SNDPLAY touche2
            'IF choixTOUCHE = 3 THEN _SNDPLAY touche3
            GOTOU = 1
        END IF
    ...
    IF GOTOU = 0 AND joueurA = 6191 THEN 'tombe1
        ...
        END IF
		'IF GOTOU = 0 AND temps > reftemps + 25 THEN
			''IF musiqueLOOP = 0 THEN SNDZEROPLAY(1, strptr(sfx_pied), (strptr(sfx_pied_end)-strptr(sfx_pied)+3) and 0xfffffffc, 46168/9233, Zero_Audio_8bit_muLaw|0)			
        'END IF
        ...
        IF GOTOU = 0 AND temps > reftemps + 2 THEN
            xA = xA - 2
            '_SNDPLAY tombe
			IF musiqueLOOP = 0 AND joueurB != 611 THEN SNDZEROPLAY(1, strptr(sfx_tombe), (strptr(sfx_tombe_end)-strptr(sfx_tombe)+3) and 0xfffffffc, 46168/9233, Zero_Audio_8bit_muLaw|0)
            'IF joueurB = 611 AND musiqueLOOP = 0 THEN SNDZEROPLAY(1, 0, 0, 0, 0) ' pour aretter les sons
            IF joueurB = 617 AND musiqueLOOP = 0 THEN SNDZEROPLAY(1, strptr(sfx_tombe), (strptr(sfx_tombe_end)-strptr(sfx_tombe)+3) and 0xfffffffc, 46168/9233, Zero_Audio_8bit_muLaw|0)
            GOTOU = 1
        END IF
    ...
    IF GOTOU = 0 AND joueurA = 9191 THEN 'tombe1r
        ...
        IF GOTOU = 0 AND temps > reftemps + 2 THEN
            xA = xA + 2
            '_SNDPLAY tombe
			IF musiqueLOOP = 0 AND joueurB != 911 THEN SNDZEROPLAY(1, strptr(sfx_tombe), (strptr(sfx_tombe_end)-strptr(sfx_tombe)+3) and 0xfffffffc, 46168/9233, Zero_Audio_8bit_muLaw|0)
            'IF joueurB = 911 AND musiqueLOOP = 0 THEN SNDZEROPLAY(1, 0, 0, 0, 0) ' pour aretter les sons
            IF joueurB = 917 AND musiqueLOOP = 0 THEN SNDZEROPLAY(1, strptr(sfx_tombe), (strptr(sfx_tombe_end)-strptr(sfx_tombe)+3) and 0xfffffffc, 46168/9233, Zero_Audio_8bit_muLaw|0)
            GOTOU = 1
        END IF
        IF GOTOU = 0 AND temps >= reftemps THEN spriteA = 45: GOTOU = 1
    END IF
...
'coco
    ...
    IF GOTOU = 0 AND joueurA = 610 THEN 'mort
        ...
        IF GOTOU = 0 AND temps > reftemps + 14 THEN GOTOU = 1
        IF GOTOU = 0 AND temps > reftemps + 13 THEN	GOTOU = 1 '_SNDPLAY mortKO:
    ...
    IF GOTOU = 0 AND joueurA = 65 THEN 'mortdecap
        ...
        IF GOTOU = 0 AND temps >= reftemps THEN
			...
			'_SNDPLAY mortdecap
			IF musiqueLOOP = 0 THEN SNDZEROPLAY(1, strptr(sfx_decapite), (strptr(sfx_decapite_end)-strptr(sfx_decapite)+3) and 0xfffffffc, 46168/9233, Zero_Audio_8bit_muLaw|0)
        END IF
        ...
        IF IA = 0 OR IA = 1 OR IA = 2 OR IA = 3 OR IA = 6 THEN
            ...
            IF GOTOU = 0 AND sens = 9 THEN
                ...
                IF GOTOU = 0 AND distance > 9 AND distance < 15 THEN ' pour se rapprocher
                    ...
                    'IF GOTOU = 0 THEN levier2 = 6: GOTOU = 20 ' bug MOONWALK
					IF GOTOU = 0 THEN joueurB = 915: reftempsB = temps: Boccupe = 1: GOTOU = 24
                END IF
            ...
            IF IA = 4 OR IA = 5 OR IA = 7 THEN
                ...
                IF sens = 6 THEN
                    ...
                    IF GOTOU = 0 AND distance = 9 THEN
                        ...
                        IF GOTOU = 0 AND joueurA = 6150 THEN joueurB = 68: reftempsB = temps: Boccupe = 1: GOTOU = 24
                        'IF GOTOU = 0 AND IA < 7 THEN
                        '    IF GOTOU = 0 AND Aoccupe = 1 THEN joueurB = 615: reftempsB = temps: Boccupe = 1: GOTOU = 24
                        'END IF
                        IF GOTOU = 0 AND IA = 7 THEN
                ...
                IF GOTOU = 0 AND sens = 9 THEN
                    ...
                    'IF GOTOU = 0 AND IA = 7 THEN
                    'IF GOTOU=0 and joueurA = 9140 THEN joueurB = 915: reftempsB = temps: Boccupe = 1: GOTOU=24
                    'END IF
                    'END IF
                    ...
                    IF GOTOU = 0 AND distance = 9 THEN
                        ...
                        IF GOTOU = 0 AND joueurA = 9150 THEN joueurB = 98: reftempsB = temps: Boccupe = 1: GOTOU = 24
                        'IF GOTOU = 0 AND IA < 7 THEN
                        'IF GOTOU=0 and Aoccupe = 1 THEN joueurB = 915: reftempsB = temps: Boccupe = 1: GOTOU=24
                        'END IF
                        IF GOTOU = 0 AND IA = 7 THEN
                    ...
                    IF GOTOU = 0 AND distance < 9 AND distance > 6 THEN ' distance de combat 1
                        ' pour autoriser les croisements
                        IF GOTOU = 0 AND demo = 0 AND joueurA = 915 THEN joueurB = 916: reftempsB = temps: Boccupe = 1: GOTOU = 24
                        ...
                        IF GOTOU = 0 AND IA = 5 THEN
                            IF GOTOU = 0 AND joueurA = 97 THEN joueurB = 913: reftempsB = temps: GOTOU = 24
                        END IF
                        ...
                        IF GOTOU = 0 AND infoBdegatT > 2 THEN
                            IF GOTOU = 0 AND joueurA = 93 THEN joueurB = 98: reftempsB = temps: Boccupe = 1: GOTOU = 24
                        END IF

                        IF GOTOU = 0 AND infoBdegatF > 2 THEN
                            IF GOTOU = 0 AND IA < 7 THEN
                                IF GOTOU = 0 AND joueurA = 97 THEN joueurB = 915: reftempsB = temps: Boccupe = 1: GOTOU = 24
                            END IF
                        END IF
                    ...
                    IF GOTOU = 0 AND distance <= 6 THEN
                        ...
                        ' pour autoriser les croisements
                        IF GOTOU = 0 AND demo = 0 AND joueurA = 916 THEN joueurB = 915: reftempsB = temps: Boccupe = 1: GOTOU = 24


                        IF GOTOU = 0 AND IA > 4 THEN
                            IF GOTOU = 0 AND joueurA = 96 THEN joueurB = 912: reftempsB = temps: GOTOU = 24
                        END IF

                        IF GOTOU = 0 AND IA > 4 AND IA < 7 THEN
                            IF GOTOU = 0 AND joueurA = 98 THEN joueurB = 916: reftempsB = temps: Boccupe = 1: GOTOU = 24
                        END IF

                        IF GOTOU = 0 AND infoBdegatG > 4 THEN
                            IF GOTOU = 0 AND joueurA = 9102 OR joueurA = 98 OR joueurA = 918 THEN joueurB = 918: reftempsB = temps: Boccupe = 1: GOTOU = 24
                        END IF
                        IF GOTOU = 0 AND infoBdegatG > 2 THEN
                            IF GOTOU = 0 AND joueurA = 911 THEN joueurB = 911: reftempsB = temps: Boccupe = 1: GOTOU = 24
                            IF GOTOU = 0 THEN
                                IF joueurA = 9102 OR joueurA = 98 OR joueurA = 918 THEN joueurB = 915: reftempsB = temps: Boccupe = 1: GOTOU = 24
                            END IF
                        END IF
    ...
    IF GOTOU = 0 AND joueurB = 61 THEN ' attente
        ...
        IF GOTOU = 0 AND temps > reftempsB + 20 THEN spriteB = 2: GOTOU = 2
        IF GOTOU = 0 AND temps > reftempsB + 11 THEN GOTOU = 2
        IF GOTOU = 0 AND temps > reftempsB + 10 THEN spriteB = 1: GOTOU = 2 '_SNDPLAY attente: 
        GOTOU = 2
    END IF
    ...
    IF GOTOU = 0 AND joueurB = 991 THEN
        xFB = xB + 4: xTB = xB + 4: xMB = xB + 4: xGB = xB + 4
        xAttB = xB
        IF GOTOU = 0 AND attaqueB = 1 THEN joueurB = 96: Boccupe = 1: reftempsB = temps: GOTOU = 24
        IF GOTOU = 0 AND temps > reftempsB + 9 THEN xB = xB + 1: spriteB = 5: spriteavanceB = 2
    ...
    IF GOTOU = 0 AND joueurB = 992 THEN
        xFB = xB + 4: xTB = xB + 4: xMB = xB + 4: xGB = xB + 4
        xAttB = xB
        spriteB = 5
        IF GOTOU = 0 AND attaqueB = 1 THEN joueurB = 96: Boccupe = 1: reftempsB = temps: GOTOU = 24
    ...
    IF GOTOU = 0 AND joueurB = 993 THEN
        xFB = xB + 4: xTB = xB + 4: xMB = xB + 4: xGB = xB + 4
        xAttB = xB
        spriteB = 6
        IF GOTOU = 0 AND attaqueB = 1 THEN joueurB = 96: Boccupe = 1: reftempsB = temps: GOTOU = 24
    ...
    IF GOTOU = 0 AND joueurB = 994 THEN
        xFB = xB + 4: xTB = xB + 4: xMB = xB + 4: xGB = xB + 4
        xAttB = xB
        spriteB = 0
        IF GOTOU = 0 AND attaqueB = 1 THEN joueurB = 96: Boccupe = 1: reftempsB = temps: GOTOU = 24
    ...
    IF GOTOU = 0 AND joueurB = 7 THEN
        ...
        xAttB = xB + 4
        IF GOTOU = 0 AND attaqueB = 1 AND sortieB = 0 THEN joueurB = 61400: Boccupe = 1: reftempsB = temps: GOTOU = 24
        spriteB = 6: spritereculeB = 1: xB = xB + 1
        GOTOU = 2
    END IF

    IF GOTOU = 0 AND joueurB = 8 THEN
        xFB = xB: xTB = xB: xMB = xB: xGB = xB
        xAttB = xB + 4
        IF GOTOU = 0 AND attaqueB = 1 AND sortieB = 0 THEN joueurB = 61400: Boccupe = 1: reftempsB = temps: GOTOU = 24
    ...
    IF GOTOU = 0 AND joueurB = 9 THEN
        ...
        GOTOU = 2
    END IF
    IF GOTOU = 0 AND joueurB = 10 THEN
        xFB = xB: xTB = xB: xMB = xB: xGB = xB
        xAttB = xB + 4
        spriteB = 4
        IF GOTOU = 0 AND attaqueB = 1 AND sortieB = 0 THEN joueurB = 61400: Boccupe = 1: reftempsB = temps: GOTOU = 24
        ...
        GOTOU = 2
    END IF
    IF GOTOU = 0 AND joueurB = 11 THEN
        xFB = xB: xTB = xB: xMB = xB: xGB = xB
        xAttB = xB + 4
        spriteB = 0
        IF GOTOU = 0 AND attaqueB = 1 AND sortieB = 0 THEN joueurB = 61400: Boccupe = 1: reftempsB = temps: GOTOU = 24
    ...
    IF GOTOU = 0 AND joueurB = 9901 THEN
        xAttB = xB
        xFB = xB + 4: xTB = xB + 4: xMB = xB + 4: xGB = xB + 4
        IF GOTOU = 0 AND attaqueB = 1 AND sortieB = 0 THEN joueurB = 9140: Boccupe = 1: reftempsB = temps: GOTOU = 24
        IF GOTOU = 0 AND temps > reftempsB + 9 THEN xB = xB - 1: spriteB = 5: spritereculeB = 2
        GOTOU = 2
    END IF
    IF GOTOU = 0 AND joueurB = 9902 THEN
        xAttB = xB
        xFB = xB + 4: xTB = xB + 4: xMB = xB + 4: xGB = xB + 4
        spriteB = 5
        IF GOTOU = 0 AND attaqueB = 1 AND sortieB = 0 THEN joueurB = 9140: Boccupe = 1: reftempsB = temps: GOTOU = 24
        ...
        GOTOU = 2
    END IF
    IF GOTOU = 0 AND joueurB = 9903 THEN
        xAttB = xB
        xFB = xB + 4: xTB = xB + 4: xMB = xB + 4: xGB = xB + 4
        spriteB = 4
        IF GOTOU = 0 AND attaqueB = 1 AND sortieB = 0 THEN joueurB = 9140: Boccupe = 1: reftempsB = temps: GOTOU = 24
        ...
        GOTOU = 2
    END IF
    IF GOTOU = 0 AND joueurB = 9904 THEN
        xAttB = xB
        xFB = xB + 4: xTB = xB + 4: xMB = xB + 4: xGB = xB + 4
        spriteB = 0
        IF GOTOU = 0 AND attaqueB = 1 AND sortieB = 0 THEN joueurB = 9140: Boccupe = 1: reftempsB = temps: GOTOU = 24
    ...
    IF GOTOU = 0 AND joueurB = 615 THEN ' rouladeAV
        ...
        IF GOTOU = 0 AND temps > reftempsB + 1 THEN xB = xB - 1: spriteB = 11: GOTOU = 2 '_SNDPLAY roule: 
    ...
    IF GOTOU = 0 AND joueurB = 6150 THEN 'rouladeAR
        ...
        IF GOTOU = 0 AND temps > reftempsB + 2 THEN GOTOU = 2
        IF GOTOU = 0 AND temps > reftempsB + 1 THEN xB = xB + 1: spriteB = 16: GOTOU = 2'_SNDPLAY roule:
    ...
    IF GOTOU = 0 AND joueurB = 6131 THEN 'protegeH1
        ...
        IF GOTOU = 0 AND temps > reftempsB + 2 THEN GOTOU = 2
        IF GOTOU = 0 AND temps > reftempsB + 1 THEN xB = xB + 1: spriteB = 4: GOTOU = 2 '_SNDPLAY protege: 
    ...
    IF GOTOU = 0 AND joueurB = 6121 THEN ' protegeD1
        ...
        IF GOTOU = 0 AND temps > reftempsB + 2 THEN GOTOU = 2
        IF GOTOU = 0 AND temps > reftempsB + 1 THEN GOTOU = 2 '_SNDPLAY protege: 
        IF GOTOU = 0 THEN GOTOU = 2
    END IF
    ...
    IF GOTOU = 0 AND joueurB = 68 THEN 'genou
        xFB = xB + 4: xTB = xB + 4: xMB = xB + 4: xGB = xB
        yGB = 20
        IF GOTOU = 0 AND temps > reftempsB + 30 THEN Boccupe = 0: joueurB = 61002: assisB = 1: GOTOU = 2
    ...
    IF GOTOU = 0 AND joueurB = 98 THEN ' genouR
        xFB = xB: xTB = xB: xMB = xB: xGB = xB + 4
        yGB = 20
        IF GOTOU = 0 AND temps > reftempsB + 30 THEN Boccupe = 0: joueurB = 9102: assisB = 1: GOTOU = 2
    ...
    IF GOTOU = 0 AND joueurB = 61400 THEN 'decapite
        ...
        IF GOTOU = 0 AND temps >= reftempsB THEN spriteB = 37: GOTOU = 2
    END IF
    ...
    IF GOTOU = 0 AND joueurB = 6191 THEN 'tombe1
        ...
        IF GOTOU = 0 AND temps > reftempsB + 2 THEN
            xB = xB + 2
            '_SNDPLAY tombe:
			IF musiqueLOOP = 0 AND joueurA != 611 THEN SNDZEROPLAY(1, strptr(sfx_tombe), (strptr(sfx_tombe_end)-strptr(sfx_tombe)+3) and 0xfffffffc, 46168/9233, Zero_Audio_8bit_muLaw|0)
            'IF joueurA = 611 AND musiqueLOOP = 0 THEN SNDZEROPLAY(1, 0, 0, 0, 0) ' pour aretter les sons
            IF joueurA = 617 AND musiqueLOOP = 0 THEN SNDZEROPLAY(1, strptr(sfx_tombe), (strptr(sfx_tombe_end)-strptr(sfx_tombe)+3) and 0xfffffffc, 46168/9233, Zero_Audio_8bit_muLaw|0)
            GOTOU = 2
        END IF
    ...
    IF GOTOU = 0 AND joueurB = 9191 THEN 'tombe1R
        ...
        IF GOTOU = 0 AND temps > reftempsB + 2 THEN
            xB = xB - 2
            '_SNDPLAY tombe
			IF musiqueLOOP = 0 AND joueurA != 911 THEN SNDZEROPLAY(1, strptr(sfx_tombe), (strptr(sfx_tombe_end)-strptr(sfx_tombe)+3) and 0xfffffffc, 46168/9233, Zero_Audio_8bit_muLaw|0)
            'IF joueurA = 911 AND musiqueLOOP = 0 THEN SNDZEROPLAY(1, 0, 0, 0, 0) ' pour aretter les sons
            IF joueurA = 917 AND musiqueLOOP = 0 THEN SNDZEROPLAY(1, strptr(sfx_tombe), (strptr(sfx_tombe_end)-strptr(sfx_tombe)+3) and 0xfffffffc, 46168/9233, Zero_Audio_8bit_muLaw|0)
            GOTOU = 2
        END IF
        IF GOTOU = 0 AND temps >= reftempsB THEN spriteB = 45: GOTOU = 2
    END IF
    ...
    IF GOTOU = 0 AND joueurB = 610 THEN 'mort
        ...
        IF GOTOU = 0 AND temps > reftempsB + 14 THEN GOTOU = 2
        IF GOTOU = 0 AND temps > reftempsB + 13 THEN GOTOU = 2 '_SNDPLAY mortKO: 
    ...
    IF GOTOU = 0 AND joueurB = 666 THEN
        ...
        IF GOTOU = 0 AND temps > reftempsB + 270 THEN xAttB = 34: GOTOU = 2
        IF GOTOU = 0 AND temps > reftempsB + 260 THEN xAttB = 9: GOTOU = 2
        IF GOTOU = 0 AND temps > reftempsB + 250 THEN xAttB = 11: GOTOU = 2
        IF GOTOU = 0 AND temps > reftempsB + 240 THEN xAttB = 14: GOTOU = 2
        IF GOTOU = 0 AND temps > reftempsB + 230 THEN xAttB = 17: GOTOU = 2
        IF GOTOU = 0 AND temps > reftempsB + 220 THEN xAttB = 20: GOTOU = 2
        IF GOTOU = 0 AND temps > reftempsB + 210 THEN xAttB = 23: spriteB = 55: GOTOU = 2
        IF GOTOU = 0 AND temps > reftempsB + 200 THEN xAttB = 26: GOTOU = 2
        ...
        IF GOTOU = 0 AND temps > reftempsB + 140 THEN xAttB = 34: GOTOU = 2
        IF GOTOU = 0 AND temps > reftempsB + 130 THEN xAttB = 9: GOTOU = 2
        IF GOTOU = 0 AND temps > reftempsB + 120 THEN xAttB = 11: GOTOU = 2
        IF GOTOU = 0 AND temps > reftempsB + 110 THEN xAttB = 14: GOTOU = 2
        IF GOTOU = 0 AND temps > reftempsB + 100 THEN xAttB = 17: GOTOU = 2
        IF GOTOU = 0 AND temps > reftempsB + 90 THEN xAttB = 20: yAttB = 20: GOTOU = 2
        IF GOTOU = 0 AND temps > reftempsB + 80 THEN xAttB = 23: spriteB = 55: yAttB = 18: GOTOU = 2
        IF GOTOU = 0 AND temps > reftempsB + 70 THEN xAttB = 26: GOTOU = 2
...
'                         *****************************************
        ...
        IF GOTOU = 0 AND joueurB = 777 THEN ' victoire */

            IF temps > reftempsB + 100 THEN
                xB = 19
				spriteB = 59
				joueurB = 770 ' sprite de FIN 
                
				xA = 0
				joueurA = 770 ' joueurA caché

				IF lancerintro = 0 THEN
					lancerintro = 1 
					'IF musiqueLOOP = 0 THEN 
						SNDZEROPLAY(1, (void *)(sfx_theme), (sfx_theme_end-sfx_theme)+3 and 0xfffffffc, 46168/7694, Zero_Audio_8bit_muLaw|Zero_Audio_Looping)
						' speed of music at 46168/11542 for emulators
						' speed of music at 46168/7694 for jaguar
					'END IF
				END IF
				
                jeu = 2: GOTOU = 31 ' jeu fini
            END IF

			IF GOTOU = 0 AND temps > reftempsB + 51 THEN GOTOU = 2 
			IF GOTOU = 0 AND temps > reftempsB + 50 THEN 
				spriteB = 58 ' drax4 */
				IF musiqueLOOP = 0 THEN SNDZEROPLAY(1, strptr(sfx_sorcier), (strptr(sfx_sorcier_end)-strptr(sfx_sorcier)+3) and 0xfffffffc, 46168/9233, Zero_Audio_8bit_muLaw|0)
				GOTOU = 2
			END IF
			IF GOTOU = 0 AND temps > reftempsB + 1 THEN GOTOU = 2           
			IF GOTOU = 0 AND temps > reftempsB THEN 
				spriteA = 0: spriteB = 57  ' drax3 */
				IF musiqueLOOP = 0 THEN SNDZEROPLAY(1, strptr(sfx_sorcier), (strptr(sfx_sorcier_end)-strptr(sfx_sorcier)+3) and 0xfffffffc, 46168/9233, Zero_Audio_8bit_muLaw|0)
				GOTOU = 2 
			END IF
    ...
    END IF
    IF GOTOU = 0 THEN
        IF sortieA = 1 OR sortieB = 1 OR jeu = 2 THEN
            IF xA < 0 THEN xA = 0
            IF xB > 40 THEN xB = 40
            GOTOU = 30
        END IF
        ...
        IF GOTOU = 0 THEN
            IF joueurA = 65 OR joueurA = 95 THEN
                ...
                IF GOTOU = 0 AND temps > reftemps + 44 THEN
                    IF xSPRt < 273 THEN
						'_SNDPLAY tete
						IF musiqueLOOP = 0 THEN SNDZEROPLAY(1, strptr(sfx_tombe), (strptr(sfx_tombe_end)-strptr(sfx_tombe)+3) and 0xfffffffc, 46168/9233, Zero_Audio_8bit_muLaw|0)					
					END IF 
    ...
    IF gnome = 1 THEN
		...
		IF gnomerigole = 0 OR finiderigoler = 1	THEN
                numerognome = 4
		...
		END IF
           ...
           'IF gnomerigole = 1 AND finiderigoler = 0 THEN

                'IF GOTOU = 0 AND temps > reftemps + 70 THEN finiderigoler = 1: reftemps = temps: GOTOU = 31 
                'IF GOTOU = 0 AND temps > reftemps + 60 THEN numerognome = 6: GOTOU = 31 
                'IF GOTOU = 0 AND temps > reftemps + 50 THEN numerognome = 5: GOTOU = 31 
                'IF GOTOU = 0 AND temps > reftemps + 40 THEN numerognome = 6: GOTOU = 31 
                'IF GOTOU = 0 AND temps > reftemps + 30 THEN numerognome = 5: GOTOU = 31 
                'IF GOTOU = 0 AND temps > reftemps + 20 THEN numerognome = 6: GOTOU = 31 
                'IF GOTOU = 0 AND temps > reftemps + 10 THEN numerognome = 5: GOTOU = 31 
                'IF GOTOU = 0 AND temps > reftemps THEN numerognome = 6: GOTOU = 31 
    ...
    'IF GOTOU = 0 THEN
        ...
        'affichage du fond
        '_PUTIMAGE (76, 208)-(559, 367), fond

        ' cache bleu du score
        'LINE (182, 112)-(464, 134), _RGB(34, 34, 153), BF
    ...
    IF transition = 1 THEN
        ...
        IF temps > reftemps + 60 THEN transition = 0: transitionfinie = 1	
    END IF
    ...
    IF GOTOU = 0 AND sorcier = 1 THEN
        ...
        ' cache Drax 
			rlist[NoSPRcache1].x=132<<16
			rlist[NoSPRcache1].y=104<<16

        'cache marianna
        	rlist[NoSPRcache2].x=196<<16
			rlist[NoSPRcache2].y=104<<16
    ...
    IF GOTOU = 0 THEN
        IF chronometre < 1 OR chronometre > 90 THEN
            chronometre = 0
            IF partie = 2 THEN
    ...
    IF GOTOU = 0 AND partie = 1 THEN RLOCATE 156,60: RPRINT "0" : PRINT "0": RLOCATE 164,60: RPRINTINT IA

    IF GOTOU = 0 AND partie = 2 THEN

        IF chronometre >= 10 THEN RLOCATE 156,60: RPRINTINT chronometre
        IF chronometre < 10 THEN RLOCATE 156,60: RPRINT "0" : PRINT "0": RLOCATE 164,60: RPRINTINT chronometre
	...
	IF GOTOU = 0 THEN
		...
		if scoreA < 0 OR niveauscoreA > 99 THEN scoreA = 0: scoreAactuel = 0: niveauscoreA = 0: RLOCATE 100,60: RPRINT "00000"
		if scoreB < 0 OR niveauscoreB > 99 THEN scoreB = 0: scoreBactuel = 0: niveauscoreB = 0: RLOCATE 188,60: RPRINT "00000"
        ...
        if scoreAactuel != scoreA THEN
            ...
            if scoreA >=1000 THEN
                scoreA = scoreA - 1000: niveauscoreA = niveauscoreA + 1
            END IF
            RLOCATE 116,60: RPRINTINT scoreA
            if scoreA <= 99 THEN RLOCATE 116,60: RPRINT "00": RLOCATE 124,60: RPRINTINT scoreA

            'affichage des dizaines de milliers
           
            if niveauscoreA <= 9 THEN 
				RLOCATE 100,60: RPRINT "0"
				RLOCATE 108,60: RPRINTINT niveauscoreA
			END IF
			
            if niveauscoreA > 9 THEN RLOCATE 100,60: RPRINTINT niveauscoreA
		...
		if scoreBactuel != scoreB THEN
            ...
            if scoreB >=1000 THEN
                    scoreB = scoreB - 1000: niveauscoreB = niveauscoreB + 1
            END IF
            RLOCATE 204,60: RPRINTINT scoreB
            if scoreB <= 99 THEN RLOCATE 204,60: RPRINT "00": RLOCATE 212,60: RPRINTINT scoreB

            'affichage des dizaines de milliers
            
            if niveauscoreB <= 9 THEN 
				RLOCATE 188,60: RPRINT "0"
				RLOCATE 196,60: RPRINTINT niveauscoreB
			END IF
			
            if niveauscoreB > 9 THEN RLOCATE 188,60: RPRINTINT niveauscoreB
    ...
    '                                  effacement des points de vie
        ...
        IF vieactuelleA != vieA THEN
            ...
            vieactuelleA = vieA
			IF vieA = 10 THEN RLOCATE 48,20: RPRINT "("
			IF vieA = 8 THEN RLOCATE 48,20: RPRINT " "
			IF vieA = 6 THEN RLOCATE 40,20: RPRINT "("
			IF vieA = 4 THEN RLOCATE 40,20: RPRINT " "
			IF vieA = 2 THEN RLOCATE 32,20: RPRINT "("
			IF vieA = 0 THEN RLOCATE 32,20: RPRINT " "
		END IF
    
		IF vieactuelleB != vieB THEN

            vieactuelleB = vieB
			IF vieB = 10 THEN RLOCATE 284,20: RPRINT "("
			IF vieB = 8 THEN RLOCATE 284,20: RPRINT " "
			IF vieB = 6 THEN RLOCATE 276,20: RPRINT "("
			IF vieB = 4 THEN RLOCATE 276,20: RPRINT " "
			IF vieB = 2 THEN RLOCATE 268,20: RPRINT "("
			IF vieB = 0 THEN RLOCATE 268,20: RPRINT " "
		END IF
    ...
    IF GOTOU = 0 AND serpentA = 1 THEN
		...
		rlist[NoSPRserpentG].x=41<<16		
		
		numeroserpent = 1
		IF temps > refserpentA + 8 THEN numeroserpent = 2
		IF temps > refserpentA + 16 THEN numeroserpent = 1
		IF temps > refserpentA + 24 THEN numeroserpent = 0: serpentA = 0 

		IF decorS = 1 OR decorS = 2 THEN ' rouge		
			rsetobj(NoSPRserpentG,R_sprite_gfxbase,(int)strptr(SPRserpentG)+ (512 * numeroserpent ) ) ' choix de la frame dans la planche 512 = 32*(32*4)/8
		END IF
		IF decorS = 3 OR decorS = 4 THEN ' rose		
			rsetobj(NoSPRserpentG,R_sprite_gfxbase,(int)strptr(SPRserpentG)+ (512 * (numeroserpent+3) ) ) ' +3 pour avancer dans la planche rose
		END IF
    ...
    IF GOTOU = 0 AND serpentB = 1 THEN
		...
		rlist[NoSPRserpentD].x=285<<16
	
		numeroserpent = 1
		IF temps > refserpentB + 8 THEN numeroserpent = 2
		IF temps > refserpentB + 16 THEN numeroserpent = 1
		IF temps > refserpentB + 24 THEN numeroserpent = 0: serpentB = 0  

		
		rsetobj(NoSPRserpentD,R_sprite_gfxbase,(int)strptr(SPRserpentD)+ (512 * numeroserpent ) )		
              ...
              '  affichage des tetes */
                  ...
                  xSPRt = (xSPRt + 1) * 8 '  laisser en x8 pour le football ,) */
                   ...
                   positionX = xSPRt : positionY = ySPRt + 24
					...
					rlist[NoSPRtetes].y=positionY+15<<16


                   ' ombre de la tete
                   positionX = xSPRt: positionY = 182
				   rlist[NoSPRombre].x=positionX<<16
				   rlist[NoSPRombre].y=positionY+15<<16
	...
	IF gnome = 0 THEN rlist[NoSPRgnome].x=-32<<16	'pour cacher le gnome
    ...
    IF GOTOU = 0 AND gnome = 1 THEN 
		...
		' cacher le gnome
		IF xGNOME > 40 THEN rlist[NoSPRgnome].x=-32<<16
    ...
    'END IF		
        ...
        IF joueurA = 65 OR joueurA = 610 THEN
            IF xGNOME > xA + 6 THEN
                'IF gnometire = 1 AND musiqueLOOP = 0 THEN SNDZEROPLAY(1, strptr(sfx_gameover), (strptr(sfx_gameover_end)-strptr(sfx_gameover)+3) and 0xfffffffc, 46168/9233, Zero_Audio_8bit_muLaw|0)
				gnometire = 0
                xA = xA + 1
            END IF
        END IF
    ...
    '                        *************************************************************
     ...
     ' boulefeu
      if  GOTOU = 9 THEN GOTOU = 0
        ...
        if  jeu = 1 AND sorcier = 1 AND xAttB < xB + 2 THEN 



          positionX =  ( xAttB + 2 ) * 8: positionY =  ( yAttB + 5 ) * 8

          if  xAttB = 26 OR xAttB = 20 OR xAttB = 14 OR xAttB = 11 OR xAttB = 9  THEN 
          ...
          if  xAttB = 23 OR xAttB = 17 OR xAttB = 11 THEN 
				rsetobj(NoSPRfeusang,R_sprite_gfxbase,(int)strptr(SPRfeusang)+ (128 * 5 ) )
				rlist[NoSPRfeusang].x=positionX<<16
				rlist[NoSPRfeusang].y=positionY<<16
          END IF

          'if  xAttB = 20 OR xAttB = 11 THEN 
      ...
      /*                     ***********giclees de sang de decapitation ************* */
            ...
            if  joueurA = 65 OR joueurA = 95 THEN 
               ...
               END IF
                ...
                if  sangdecapA = 4 THEN 
                   positionX = ((xA + 2) * 8) + 8: positionY = ((yMA + 1) * 8)
					rsetobj(NoSPRfeusang,R_sprite_gfxbase,(int)strptr(SPRfeusang)+ (128 * 1 ) )
               END IF
            ...
            if  joueurB = 65 OR joueurB = 95 THEN 
               ...
               END IF
                ...
                if  sangdecapB = 4 THEN 
                   positionX = ((xB + 2) * 8) + 8: positionY = ((yMB + 1) * 8)
					rsetobj(NoSPRfeusang,R_sprite_gfxbase,(int)strptr(SPRfeusang)+ (128 * 1 ) )
               END IF
           ...
           END IF
				...
				rlist[NoSPRfeusang].x=positionX<<16
				rlist[NoSPRfeusang].y=positionY+8<<16
               END IF
    ...
    'IF GOTOU = 0 THEN
        ...
        'IF jeu = 2 THEN RLOCATE 112,196: RPRINT "THANKS.BIG.BOY"

        'IF jeu = 0 AND joueurA = 6660 THEN RLOCATE 92,128: RPRINT "YOUR.END.HAS.COME"
    ...
    IF arret = 1 AND GOTOU < 100 THEN
        ...
        IF temps > 30 THEN
			...
			'IF musiqueLOOP = 0 THEN SNDZEROPLAY(1, 0, 0, 0, 0) ' pour aretter les sons
			' lancer la musique pour le menu
			IF musiqueLOOP = 0 THEN SNDZEROPLAY(1, (void *)(sfx_theme), (sfx_theme_end-sfx_theme)+3 and 0xfffffffc, 46168/7694, Zero_Audio_8bit_muLaw|Zero_Audio_Looping)
			' speed of music at 46168/11542 for emulators
			' speed of music at 46168/7694 for jaguar
        ...
        IF jeu = 2 OR jeu = 0 THEN		
            partie = 1: decorS = 3: IA = 0: sorcier = 0: marianna = 0
            GOTOU = 8
        END IF
...
'if GOTOU = 0 THEN
	...
	'RLOCATE 128,8: RPRINT "BETA.125"
	'RLOCATE 90,16: RPRINT "   ":RPRINTINT xA 
	'RLOCATE 160,16: RPRINT "   ":RPRINTINT yTA


	'RLOCATE 60,8: RPRINT "      ": RPRINTINT sonpoints
	'RLOCATE 82,8: RPRINT "      ": RPRINTINT GOTOU
	'RLOCATE 160,8: RPRINT "      ": RPRINTINT y1
'END IF	
...
' remettre la musique à la bonne vitesse pour le skunk



' pour la version barbarian+, faire une rom emu avec la bonne vitesse (attention la musique est lancée à 5 endroits) > fin des demo


' bug communs à toutes les versions :
' bug de entreesorcier + crier serpents
' retirer bug de cling looping sur les autres versions
' bug son moulinette sur toutes les versions !!
' verifier son decapite sur les autres versions
