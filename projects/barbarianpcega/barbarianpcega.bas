'_______________DEATH SWORD_______________
'__________palace software 1988_________
'____a remake of IBM PC-CGA version_____
...
loadclut(strptr(decorsPC_clut),0,16) ' palette decors
...
DO
    ...
    IF GOTOU = 101 THEN
		...
		if temps > 400 THEN 
			temps = 0: GOTOU = 100	
			' choix du titre
			rsetobj(NoSPRtitre,R_sprite_gfxbase,(int)strptr(decorsPC)+ (32000) )' choix de la frame dans la planche 32000 = 320*(200*4)/8
		END IF
	END IF
    ...
    IF GOTOU = 100 THEN
		...
		IF loading = 1 THEN
			...
			temps=temps+1
			'RLOCATE 128,4: RPRINTINT temps
			if temps > 5150 THEN temps = 0: loading = 0: GOTOU = 8
    ...
    IF GOTOU = 32 THEN
			...
			' choix du decor
			rsetobj(NoSPRdecor,R_sprite_gfxbase,(int)strptr(decorsPC)+ (32000 * (decorS+1) ) ) ' choix de la frame dans la planche 32000 = 320*(200*4)/8

			
		'  affichage de l'arbre gauche
		rsetobj(NoSPRarbreG,R_sprite_gfxbase,(int)strptr(arbresGAUCHE)+ 1536* (decorS-1) ) ' choix de la frame dans la planche 1536 = 64*(48*4)/8
		rlist[NoSPRarbreG].y=136+8<<16
			
		'  affichage de l'arbre droit
		rsetobj(NoSPRarbreD,R_sprite_gfxbase,(int)strptr(arbresDROIT)+ 1536* (decorS-1) )			
		rlist[NoSPRarbreD].y=136+8<<16		

		
		' re-afficher les sprites
		rlist[NoSPRspriteA].y=120+8<<16	'joueurA
		rlist[NoSPRspriteB].y=120+8<<16	'joueurB		


		' affichage des scores
		if scoreA = 0 THEN RLOCATE 100,56: RPRINT "00000"
		if scoreB = 0 THEN RLOCATE 188,56: RPRINT "00000"
...
sonrigole = 0
    ...
    IF GOTOU = 0 AND marianna = 1 THEN
            ...
            IF GOTOU = 0 THEN GOTOU = 23 ' degatsA */
	END IF
...
'coco
    ...
    IF GOTOU = 0 AND sorcier = 1 THEN
        ...
        ' cache Drax
				rlist[NoSPRcache1].x=124<<16
				rlist[NoSPRcache1].y=96+8<<16
    END IF
	
	
        'cache marianna
        IF GOTOU = 0 AND marianna = 1 THEN
				rlist[NoSPRcache2].x=203<<16
				rlist[NoSPRcache2].y=96+8<<16
        END IF
    ...
    IF GOTOU = 0 AND partie = 1 THEN RLOCATE 156,56: RPRINT "0" : PRINT "0": RLOCATE 164,56: RPRINTINT IA

    IF GOTOU = 0 AND partie = 2 THEN

        IF chronometre >= 10 THEN RLOCATE 156,56: RPRINTINT chronometre
        IF chronometre < 10 THEN RLOCATE 156,56: RPRINT "0" : PRINT "0": RLOCATE 164,56: RPRINTINT chronometre
	...
	IF GOTOU = 0 THEN
		...
		if scoreA < 0 OR niveauscoreA > 99 THEN scoreA = 0: scoreAactuel = 0: niveauscoreA = 0: RLOCATE 100,56: RPRINT "00000"
		if scoreB < 0 OR niveauscoreB > 99 THEN scoreB = 0: scoreBactuel = 0: niveauscoreB = 0: RLOCATE 188,56: RPRINT "00000"
        ...
        if scoreAactuel != scoreA THEN
            ...
            if scoreA >=1000 THEN
                scoreA = scoreA - 1000: niveauscoreA = niveauscoreA + 1
            END IF
            RLOCATE 116,56: RPRINTINT scoreA
            if scoreA <= 99 THEN RLOCATE 116,56: RPRINT "00": RLOCATE 124,56: RPRINTINT scoreA

            'affichage des dizaines de milliers
           
            if niveauscoreA <= 9 THEN 
				RLOCATE 100,56: RPRINT "0"
				RLOCATE 108,56: RPRINTINT niveauscoreA
			END IF
			
            if niveauscoreA > 9 THEN RLOCATE 100,56: RPRINTINT niveauscoreA
		...
		if scoreBactuel != scoreB THEN
            ...
            if scoreB >=1000 THEN
                    scoreB = scoreB - 1000: niveauscoreB = niveauscoreB + 1
            END IF
            RLOCATE 204,56: RPRINTINT scoreB
            if scoreB <= 99 THEN RLOCATE 204,56: RPRINT "00": RLOCATE 212,56: RPRINTINT scoreB

            'affichage des dizaines de milliers
            
            if niveauscoreB <= 9 THEN 
				RLOCATE 188,56: RPRINT "0"
				RLOCATE 196,56: RPRINTINT niveauscoreB
			END IF
			
            if niveauscoreB > 9 THEN RLOCATE 188,56: RPRINTINT niveauscoreB
    ...
    IF GOTOU = 0 THEN
        ...
        IF joueurA = 610 OR joueurA = 910 OR joueurA = 65 OR joueurA = 95 THEN
			...
			rlist[NoSPRspriteAmort].y=120+8<<16
...
'END IF
              ...
              '  affichage des tetes */
                  ...
                  xSPRt = (xSPRt + 1) * 8 '  laisser en x8 pour le football ,) */
                   ...
                   positionX = xSPRt : positionY = ySPRt + 24
					...
					rlist[NoSPRtetes].y=positionY+8<<16


                   ' ombre de la tete
                   positionX = xSPRt: positionY = 182
				   rlist[NoSPRombre].x=positionX<<16
				   rlist[NoSPRombre].y=positionY+8<<16
    ...
    IF GOTOU = 0 AND gnome = 1 THEN 
		...
		rlist[NoSPRgnome].y=168+8<<16	'gnome
     ...
     ' boulefeu
      if  GOTOU = 9 THEN GOTOU = 0
        ...
        if  jeu = 1 AND sorcier = 1 AND xAttB < xB + 2 THEN 
          ...
          positionX =  ( xAttB + 2 ) * 8: positionY =  ( yAttB +2 ) * 8 +8
    ...
    IF GOTOU = 0 THEN
        ...
        IF jeu = 2 THEN RLOCATE 112,192:: RPRINT "THANKS.BIG.BOY"
